 ...
 */
    ...
    var ElementsOccurringKTimes = function(arr, k){
        var le = Math.floor(arr.length/k);
        var p = [le];
        ...
        for(var  i=0; i < k ; i++){
            if(p[i].count > 0){
                ...
                if(count >= arr.length/k){
                    System.out.prvar ln(p[i].element);
                }
            }
        }
    }
        ...
        var  arr= [3,2,2,1,1,2,3,3,4,5,3,1];
        ElementsOccurringKTimes(arr, 3);
