...
function spanningTree(g, source) {
   var distanceTable = [];
   var spanningT ={};
   var visitedVert = [];
   ...
   while(!queue.isEmpty()){
      ...
      if( visitedVert[currentV] ) continue;
      visitedVert[currentV] = 1;
      if(currentV !== source) {
         var lastV = distanceTable[currentV].prceeding;
         var edge = lastV + "-->"+currentV;
         if(!spanningT[edge]) {
            spanningT[edge] = 1;
            console.log(edge);
         }
      ...
      for(var j=0; j<v.length; j++) {
         var distance = g.get_edgeW(currentV, v[j])
   ...
   console.log("");
   console.log("");
   for (var k in spanningT)
   console.log(k);
}

var g1 = new Graph(9,1);
g1.add(0,1,1);
g1.add(1,2,2);
g1.add(1,3,2);
g1.add(2,3,2);
...
//Prims Algo greedy algo find local best not global best, it finds spanning tree from start node
//Intermidiate result is a tree as well, does not work for disconnected graph
//also called PrimsDikstra algo. O(E ln V) BinaryHeap  Vs Array O(E + V^2); 
